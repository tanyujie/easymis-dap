...
public class OperateLogAspect {
    @Autowired
    private OperateLogInfoGeneration logInfoGeneration;

    @Autowired
    private OperateLogService logManager;

    @Pointcut("execution(* org.easymis.dap.catalog.service..*.*(..))")
    ...
    public Object aroundManagerLogPoint(ProceedingJoinPoint jp) throws Throwable {
        ...
        String operationTypeName = logEventMethod.event().getEvent();
        String objectName = logEventMethod.module().getModule();
        String depict = logEventMethod.desc();

        if(logEventClass != null){
            // 如果方法上的值为默认值，则使用全局的值进行替换
        	operationTypeName = operationTypeName.equals(EventType.DEFAULT) ? logEventClass.event().getEvent() : operationTypeName;
        	objectName = objectName.equals(ModuleType.DEFAULT) ? logEventClass.module().getModule() : objectName;
        }

        OperateLog logBean = new OperateLog();
        logBean.setObjectName(objectName);
        logBean.setOperationTypeName(operationTypeName);
        logBean.setDepict(depict);
        logBean.setCreateDate(new Date());
        logInfoGeneration.processingManagerLogMessage(jp, logBean, method);
        Object returnObj = jp.proceed();

        if(operationTypeName.equals(EventType.LOGIN)){
            //TODO 如果是登录，还需要根据返回值进行判断是不是成功了，如果成功了，则执行添加日志。这里判断比较简单
            if(returnObj != null) {
               // this.logManager.dealLog(logBean);
            }
        }else {
            this.logManager.save(logBean);
        }
        return returnObj;
    }
